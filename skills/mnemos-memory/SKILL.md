---
name: mnemos-memory
description: Use when users ask to install, configure, troubleshoot, or operate Mnemos for persistent scoped agent memory, or when they mention Mnemos, agent memory, scoped memory, memory MCP tools, or memory automation.
---

# Mnemos Memory

Mnemos is a local-first memory layer for coding agents. Use this skill to guide users onto the supported install path, explain the operating loop, and keep compatibility claims accurate.
...
- For Codex, Cursor, and generic MCP hosts, do not imply automatic capture unless the host has its own automation or the user adds one.
- Neo4jStore is experimental. Do not present it as graph-native spreading-activation persistence yet.

## Workflow

1. Identify the host: Claude Code, Cursor, Codex, or generic MCP.
...
5. Read `references/hosts.md` for host-specific config snippets and caveats.
